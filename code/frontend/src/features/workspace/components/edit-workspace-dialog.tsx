...
import { Loader2, X, Edit3, Palette } from 'lucide-react';
...
export function EditWorkspaceDialog({ workspace, isOpen, onClose }: EditWorkspaceDialogProps) {
  ...
  useEffect(() => {
    if (workspace) {
      ...
      setThemeColor(workspace.themeColor || '#4F46E5');
    }
  }, [workspace, reset]);
      ...
      <div className="w-full max-w-md rounded-2xl border border-surface-border bg-surface p-6 shadow-2xl text-text-primary">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-text-secondary flex items-center space-x-1.5">
              <Palette className="h-3.5 w-3.5 text-primary" />
              <span>Màu chủ đề Workspace</span>
            </label>
            <div className="flex items-center space-x-2">
              {PRESET_COLORS.map((color) => (
                <button
                  ...
                  className={`h-6 w-6 rounded-full transition-transform ${
                    themeColor === color ? 'scale-125 ring-2 ring-primary ring-offset-2 ring-offset-surface' : 'hover:scale-110'
                  }`}
                />
              ))}
            </div>
